...
import Reveal from '../components/Reveal'; // Підключаємо твій компонент анімацій

const C = { yellow: '#facc15', muted: '#a1a1aa', border: '#3f3f46', bg: '#09090b', surface: '#121214' };

// Тимчасова БД турнірів
const TOURNAMENTS = [
  {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {TOURNAMENTS.map((t, index) => (
        <Reveal key={t.id} delay={index * 100} direction="up">
          <div style={{ display: 'flex', flexWrap: 'wrap', background: C.surface, borderRadius: 12, overflow: 'hidden', border: `1px solid ${C.border}`, alignItems: 'center', transition: 'transform 0.2s', cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
            ...
            <Reveal delay={200} direction="up">
              <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between', marginBottom: 60, overflowX: 'auto', paddingBottom: 8 }}>
                {[
                  ...
                  <div key={i} style={{ border: `1px solid ${C.yellow}`, borderRadius: 999, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(250, 204, 21, 0.03)', flex: 1, minWidth: 'max-content' }}>
